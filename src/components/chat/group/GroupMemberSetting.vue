          ...
          <div>
            <q-item v-for="member in members">
              ...
              <q-item-section v-if="isAdmin && myUserId != member.id" side top>
                <q-btn
                  v-if="!administrators.find((v) => v.id == member.id)"
                  @click="kickOff(member.id)"
                  flat
                  dense
                  round
                  icon="delete"
                ...
                <template
                  v-if="
                    !administrators.find((v) => v.id == member.id) &&
                    !banned.find((v) => v.id == member.id)
                  "
                >
                  <q-btn
                    @click="editAdmin(member.id, true)"
                    flat
                    dense
                    round
                  ...
                  <q-btn
                    @click="editBanned(member.id, true)"
                    flat
                    dense
                    round
                    icon="not_interested"
          ...
          <div>
            <q-item v-for="admin in administrators">
              ...
              <q-item-section v-if="isAdmin && myUserId != admin.id" side top>
                <q-btn
                  @click="editAdmin(admin.id, false)"
                  flat
                  dense
                  round
                  icon="delete"
...
export default {
  ...
  computed: {
    ...
    isAdmin() {
      return (
        this.administrators.find((a) => a.id == this.myUserId) != undefined
      );
    },
  },
  props: {
